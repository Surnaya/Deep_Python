...
Пользователь задается по id. Удалить нужно все сообщения, лайки, медиа записи, профиль и 
запись из таблицы users. Функция должна возвращать номер пользователя. */

DROP FUNCTION IF EXISTS delete_user;
DELIMITER //
CREATE FUNCTION delete_user(delete_id INT)
RETURNS INT READS SQL DATA
BEGIN

	DELETE FROM media WHERE user_id = delete_id;
    -- Удаление всех сообщений пользователя
	DELETE FROM messages WHERE from_user_id = delete_id;

	-- Удаление всех лайков пользователя
	DELETE FROM likes WHERE user_id = delete_id;

-- Удаление всех медиа записей пользователя
	DELETE FROM media WHERE user_id = delete_id;

	-- Удаление профиля пользователя
	DELETE FROM profiles WHERE user_id = delete_id;

-- Удаление записи из таблицы friend_requests
...
DELIMITER //
CREATE PROCEDURE delete_user_procedure (IN user_id INT)
BEGIN
	DECLARE exit handler for sqlexception
BEGIN
	rollback;
	select 'Произошла ошибка';
END;

START TRANSACTION;

	DELETE FROM friend_requests WHERE initiator_user_id = user_id;
	DELETE FROM friend_requests WHERE target_user_id = user_id;
...
DROP TRIGGER IF EXISTS community_name_len;
DELIMITER //
CREATE TRIGGER community_name_len
BEFORE INSERT ON communities
FOR EACH ROW
BEGIN
	IF LENGTH(NEW.name) < 5 THEN
	SIGNAL SQLSTATE '45000'
	SET MESSAGE_TEXT = 'The length of the community name must be at least 5 characters';
	END IF;
END//
DELIMITER ;